...
export default function SubAdminProviders() {
  ...
  const submitVerify = () => { if (!verifyModal) return; setActing(verifyModal.id); verifySubadminProvider(verifyModal.id, { license_number: licenseNumber || undefined, license_expiry: licenseExpiry || undefined, }).then(() => { addToast("success", "Approved", `${verifyModal.name} has been verified`); setVerifyModal(null); fetchProviders(); }).catch((e: any) => addToast("error", "Error", e?.response?.data?.error || "Failed")).finally(() => setActing(null)); };
