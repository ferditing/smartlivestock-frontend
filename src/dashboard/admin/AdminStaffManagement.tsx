  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        ...(form.role === "subadmin" && { assigned_county: form.assigned_county }),
      };
            ...
            <form onSubmit={handleCreate} className="card-body space-y-4">
              ...
              {form.role === "subadmin" && (
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
                    Sub-admin will manage all users and data for this county. One active sub-admin per county onlyâ€”suspend the current one in User Management before assigning a new one.
                  </p>
                </div>
              )}
              <button
